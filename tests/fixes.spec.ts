...
import { run, loop, fnJob, gateJob, agentCheck, predicate, MockEngine } from '../src/api.ts';
import type { Outcome, RunOptions } from '../src/api.ts';

const mockOpts: RunOptions = { engine: 'mock', engines: { mock: () => new MockEngine(() => '') } };
const replying = (text: string): RunOptions => ({ engine: 'mock', engines: { mock: () => new MockEngine(() => text) } });
  ...
  it('threads the failed review to the next iteration as ctx.lastReview', async () => {
    ...
    await run(
      loop({
        ...
        review: fnJob('rev', async () => {
          reviews += 1;
          return reviews >= 2 ? { status: 'pass' } : { status: 'fail', summary: 'fix X' };
        }),
        max: 5,
      }),
      mockOpts,
  ...
  it('sees the enclosing loop body outcome', async () => {
    ...
    const { outcome } = await run(
      loop({
        ...
        until: predicate(() => true),
        review: gateJob('rev', predicate((_ctx, last) => last?.data === 'ready')),
        max: 5,
      }),
      mockOpts,
    );
...
describe('agentCheck robustness', () => {
  it('degrades to not-met on a malformed verdict instead of throwing', async () => {
    const { outcome } = await run(
      loop({ name: 'x', body: fnJob('b', async () => ({ status: 'fail' })), until: agentCheck({ question: '?', threshold: 0.5 }), max: 2 }),
  ...
  it('extracts the first balanced object from prose-wrapped multi-object replies', async () => {
    const { outcome } = await run(
      loop({ name: 'x', body: fnJob('b', async () => ({ status: 'fail' })), until: agentCheck({ question: '?', threshold: 0.8 }), max: 3 }),
      replying('Restating input: {"question":"?"}. My verdict: {"verdict":"yes","confidence":0.95}'),
    );
    expect(outcome.status).toBe('pass');
  });
  ...
  it('treats a "yes" with no confidence as confident', async () => {
    const { outcome } = await run(
      loop({ name: 'x', body: fnJob('b', async () => ({ status: 'fail' })), until: agentCheck({ question: '?', threshold: 0.8 }), max: 2 }),
