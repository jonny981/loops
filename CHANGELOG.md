...
- **The documentation site** (`site/`). A structured public docs site:
  hand-authored guides (getting started, core concepts, loops and gates,
  graphs, memory, engines, steering, momentum, webhooks and control, the
  CLI) built with VitePress, plus a complete API reference generated by
  TypeDoc from `src/api.ts` on every build — so the published reference can
  never drift from the source types. `npm run docs:dev|build|preview`;
  the `Docs` workflow deploys to GitHub Pages on `main` pushes. `src/api.ts`
  re-exports every type a public signature references, so the reference is a
  closed graph (TypeDoc runs warning-free); inline code renders `v-pre` so
  generated comments can never break the Vue compiler.
